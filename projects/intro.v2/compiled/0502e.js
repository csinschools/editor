...
return $loc;throw new Sk.builtin.SystemError('internal error: unterminated block');}}catch(err){ if (!(err instanceof Sk.builtin.BaseException)) { err = new Sk.builtin.ExternalError(err); } err.traceback.push({lineno: $currLineNo, colno: $currColNo, filename: '<stdin>.py'}); if ($exc.length>0) { $err = err; $blk=$exc.pop(); continue; } else { throw err; }} } });$scope0.$const2 = new Sk.builtin.str("What year did World War 1 start?");$scope0.$const6 = new Sk.builtin.str("1914");$scope0.$const10 = new Sk.builtin.str("1914 is right!");$scope0.$const13 = new Sk.builtin.str("World War 1 started in 1914 and lasted until 1918.");
return $scope0;}();
$compiledmod;