 ...
 var $loadname13=$loc.print!==undefined?$loc.print:Sk.misceval.loadname('print',$gbl);;$ret = ($loadname13.tp$call)?$loadname13.tp$call([$scope0.$const14],undefined) : Sk.misceval.applyOrSuspend($loadname13,undefined,undefined,undefined,[$scope0.$const14]);$blk=10;case 10: if ($ret && $ret.$isSuspension) { return $saveSuspension($ret,'<stdin>.py',5,0); }var $call15=$ret;

 $currLineNo = 5;
 $currColNo = 0;

 return $loc;throw new Sk.builtin.SystemError('internal error: unterminated block');}}catch(err){ if (!(err instanceof Sk.builtin.BaseException)) { err = new Sk.builtin.ExternalError(err); } err.traceback.push({lineno: $currLineNo, colno: $currColNo, filename: '<stdin>.py'}); if ($exc.length>0) { $err = err; $blk=$exc.pop(); continue; } else { throw err; }} } });$scope0.$const2 = new Sk.builtin.str("Facts about Australia!");$scope0.$const5 = new Sk.builtin.str("Press [Enter] to display a fact");$scope0.$const8 = new Sk.builtin.str("Australia is the only country which is also a continent.");$scope0.$const11 = new Sk.builtin.str("Press [Enter] to display another fact");$scope0.$const14 = new Sk.builtin.str("The word 'selfie' was invented in Australia!");
 return $scope0;}();
 $compiledmod;