...
var $loadname4=$loc.input!==undefined?$loc.input:Sk.misceval.loadname('input',$gbl);;$ret = ($loadname4.tp$call)?$loadname4.tp$call([$scope0.$const5],undefined) : Sk.misceval.applyOrSuspend($loadname4,undefined,undefined,undefined,[$scope0.$const5]);$blk=4;case 4: if ($ret && $ret.$isSuspension) { return $saveSuspension($ret,'<stdin>.py',2,8); }var $call6=$ret;

$currLineNo = 2;
$currColNo = 8;

$loc.movie=$call6;if (Sk.breakpoints('<stdin>.py',4,0)) {var $susp = $saveSuspension({data: {type: 'Sk.debug'}, resume: function() {}}, '<stdin>.py',4,0);$susp.$blk = 5;$susp.optional = true;return $susp;}$blk=5;case 5: 

$currLineNo = 4;
$currColNo = 0;

var $loadname7=$loc.print!==undefined?$loc.print:Sk.misceval.loadname('print',$gbl);;var $loadname9=$loc.movie!==undefined?$loc.movie:Sk.misceval.loadname('movie',$gbl);;var $binop10=Sk.abstr.numberBinOp($scope0.$const8,$loadname9,'Add');$ret = ($loadname7.tp$call)?$loadname7.tp$call([$binop10],undefined) : Sk.misceval.applyOrSuspend($loadname7,undefined,undefined,undefined,[$binop10]);$blk=6;case 6: if ($ret && $ret.$isSuspension) { return $saveSuspension($ret,'<stdin>.py',4,0); }var $call11=$ret;

$currLineNo = 4;
$currColNo = 0;

return $loc;throw new Sk.builtin.SystemError('internal error: unterminated block');}}catch(err){ if (!(err instanceof Sk.builtin.BaseException)) { err = new Sk.builtin.ExternalError(err); } err.traceback.push({lineno: $currLineNo, colno: $currColNo, filename: '<stdin>.py'}); if ($exc.length>0) { $err = err; $blk=$exc.pop(); continue; } else { throw err; }} } });$scope0.$const2 = new Sk.builtin.str("Welcome to NetFlicks!");$scope0.$const5 = new Sk.builtin.str("What movie would you like to watch? ");$scope0.$const8 = new Sk.builtin.str("Now playing: ");
return $scope0;}();
$compiledmod;