...
return $loc;throw new Sk.builtin.SystemError('internal error: unterminated block');}}catch(err){ if (!(err instanceof Sk.builtin.BaseException)) { err = new Sk.builtin.ExternalError(err); } err.traceback.push({lineno: $currLineNo, colno: $currColNo, filename: '<stdin>.py'}); if ($exc.length>0) { $err = err; $blk=$exc.pop(); continue; } else { throw err; }} } });$scope0.$const2 = new Sk.builtin.str("Login Screen");$scope0.$const5 = new Sk.builtin.str("Type in your password, then press [Enter]:");$scope0.$const8 = new Sk.builtin.str("Welcome to my online store!");
return $scope0;}();
$compiledmod;